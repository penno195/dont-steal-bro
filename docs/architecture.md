...
| **UI framework** | **Vide** (`centau/vide`), chosen in P6-2 over React-lua and Fusion. | Declarative UI is the only practical way to keep 15+ task screens consistent. Vide's fine-grained sources map straight onto the controllers' `observe(fn)` surface, and it type-checks under `--!strict`. See `docs/components.md`. |
