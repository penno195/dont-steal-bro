...
| 8 | Vote board | Colours | Hard-coded `COLORS` table, so it ignores the theme and the colour-blind mode. The tie and winner highlights are colour-only apart from their tag text. | Medium | Move to `Theme.color` during the #6 rebuild. P6-8 owns the colour-blind signalling. |
